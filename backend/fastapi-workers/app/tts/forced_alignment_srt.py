...
class CharacterTiming:
    """Forced Alignment가 돌려준 실제 발음 문자 하나의 시점."""
...
def align_audio_to_text(
    ...
    *,
    ...
    payload = r.json()
    # 문자 시점이 있으면 공백을 제외해 단어 단위로 병합한다. 일부 응답의
    # words 배열은 공백도 별도 항목으로 반환하므로 그대로 자막 큐에 넣지 않는다.
    raw_characters = payload.get("characters") or []
    if raw_characters and isinstance(raw_characters[0], dict):
        out = _chars_to_words(
            [str(character.get("text", "")) for character in raw_characters],
            [float(character.get("start", 0.0)) for character in raw_characters],
            [float(character.get("end", 0.0)) for character in raw_characters],
        )
    else:
        out = [
            WordTiming(
...
def align_audio_to_text_with_characters(
    audio_path: str,
    transcript: str,
    *,
    api_key: Optional[str] = None,
) -> tuple[list[WordTiming], list[CharacterTiming]]:
    """실제 발음 문자 시점과 단어 시점을 함께 반환한다.

    문자 시점이 제공되면 화면 자막의 끝을 숫자 발음 등의 마지막 실제 문자에
    맞출 수 있다. 제공자가 문자 배열을 생략한 경우에도 단어 배열은 폴백으로
    반환해 기존 정렬 흐름을 유지한다.
    """
    key = api_key or os.environ.get("ELEVENLABS_API_KEY")
    if not key:
        # 기존 단어 정렬 경로와 같은 오류·폴백 계약을 유지한다.
        if api_key is None:
            return align_audio_to_text(audio_path, transcript), []
        return align_audio_to_text(audio_path, transcript, api_key=api_key), []
    ...
    if response.status_code != 200:
        raise AlignmentError(f"정렬 실패 {response.status_code}: {response.text[:300]}")
    ...
    if raw_characters and isinstance(raw_characters[0], dict):
        characters = [
            CharacterTiming(
                text=str(item.get("text", item.get("character", ""))),
                start=float(item.get("start", 0.0)),
                end=float(item.get("end", 0.0)),
            )
            for item in raw_characters
        ]
        characters = [item for item in characters if item.text]

    if characters:
        words = _chars_to_words(
            [item.text for item in characters],
            [item.start for item in characters],
            [item.end for item in characters],
        )
    else:
        words = [
            WordTiming(
    ...
    if not words:
        raise AlignmentError(f"정렬 결과가 비어 있습니다. 응답 키: {list(payload.keys())}")
    ...
    *,
    ...
    for i, w in enumerate(words):
        if buf:
            ...
            next_len = len(cur_text) + 1 + len(w.text)
            if gap >= gap_split or next_len > max_chars or cur_dur >= max_duration:
                flush()
        buf.append(w)
    flush()
    return cues
