...
def _logical_support_metrics(text: str, px: int = 56) -> tuple[float, float]:
    ...
    # p50 reflects the usable interior stroke after rasterization.  p10 is
    # dominated by the antialiased boundary pixel and cannot represent a
    # readable stroke width (it remains ~1 px even for a thick glyph).
...
def diegetic_supersample_factor(
    ...
    *,
    ...
    # The render supersample is derived from the same physical floors.  It
    # improves interpolation, while the two typography scales preserve the
    # glyph geometry that a proportional canvas resize alone cannot preserve.
...
def composite_planar(image: Image.Image, chart: dict, plan: InfoSurfacePlan, detection: QuadDetection, content_override: Image.Image | None = None) -> Image.Image:
    ...
    content = content_override if content_override is not None else rendered.image
    ...
    plan.chart_render_metadata = {"bars": [] if content_override is not None else rendered.bars, "canvas_size": list(content.size)}
    ...
    warped = cv2.warpPerspective(rgba, cv2.getPerspectiveTransform(source, quad), (base.width, base.height), flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_TRANSPARENT)
    alpha = warped[:, :, 3]
    occluder = cv2.dilate(detection.occluder_mask, np.ones((5, 5), np.uint8))
    ...
    texture_alpha = (alpha.astype(np.float32) * .18).astype(np.uint8)
