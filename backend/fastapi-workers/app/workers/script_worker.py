...
🎯 녹색 지폐 캐릭터 및 비주얼 프롬프트 규칙:
- 모든 씬에 들어갈 [비주얼] 이미지 프롬프트는 아래 지침에 맞추어 영어(English)로 상세히 작성하세요.
- **주인공 캐릭터 설정**: 의인화된 녹색 지폐(한국 만 원권/천 원권 스타일) 카툰 캐릭터.
  - 외모 키워드: "A cute green banknote cartoon character with glasses and a headset, with numbers '1000' or '10000' printed on its forehead"
  - 예술 스타일: "high-quality 2D vector cartoon style, vibrant colors, clean lines, professional financial news studio or corporate office background"
- 각 씬의 대사 분위기나 감정 상태(예: 기쁨, 걱정, 놀람, 차분한 분석)를 비주얼 프롬프트의 표정(expression) 및 자세(pose)에 구체적으로 반영해 주세요.
- **칙칙한 차트나 도표 등 추상적 이미지는 절대 지양**하고, 캐릭터가 직접 칠판을 가리키거나 모니터 앞, 또는 뉴스 데스크에 있는 구체적인 2D 카툰 씬 일러스트로 작성해야 합니다.
...
## 씬 [번호]: [씬 제목]
[대사]
(여기에는 실제 한국어로 낭독할 대사를 작성하세요. 문장당 15자 이내의 아주 짧은 문장 위주로 끊어서 작성하고 마침표로 끝내야 합니다. 씬 하나당 공백 제외 120자~180자 내외로 작성하세요.)
[비주얼]
(여기에는 녹색 지폐 캐릭터가 처한 상황이나 설명하는 동작, 표정, 배경을 구체적인 영어 이미지 생성 프롬프트로 작성하세요. 예술 스타일과 캐릭터의 생김새를 포함하여 3~4문장으로 묘사하세요. 칙칙한 도표나 단순 그래프는 금지합니다.)
...
[비주얼]
A cute green banknote cartoon character with glasses and a headset, showing a worried expression, pointing at a large virtual stock market display showing down arrows, KOSPI financial board background, clean vector 2D cartoon style, 8k, cinematic lighting.
"""
        ...
        # 전체 낭독용 스크립트는 씬의 대사([대사] 부분)만 이어붙여서 구성합니다.
...
def _parse_sections(full_text: str) -> list:
    """## 씬 제목 또는 ## 섹션명 기준으로 분리하고, [대사]와 [비주얼]을 추출합니다."""
    ...
    for i in range(1, len(parts), 2):
        if i + 1 < len(parts):
            ...
            narration_match = re.search(r'\[대사\]\s*(.*?)\s*(?:\[비주얼\]|$)', raw_content, re.DOTALL)
            if narration_match:
                narration = narration_match.group(1).strip()
            else:
                narration = re.sub(r'\[비주얼\].*$', '', raw_content, flags=re.DOTALL).strip()
                
            prompt_match = re.search(r'\[비주얼\]\s*(.*?)\s*$', raw_content, re.DOTALL)
            if prompt_match:
                prompt = prompt_match.group(1).strip()
            else:
                prompt = f"A cute green banknote cartoon character with glasses and a headset, showing an expression matching '{title}', professional 2D vector style"
    ...
    if not sections:
        sections.append({
            ...
            "prompt": "A cute green banknote cartoon character with glasses and a headset, smiling, professional corporate background, 2D vector style",
