...
def _calc_scene_count(target_minutes: int) -> int:
    """목표 분량별 씬(이미지) 수 계산 — 초반 몰입도 최적화 기준"""
    if target_minutes <= 5:
        return 40
    elif target_minutes <= 10:
        return max(40, int(target_minutes * 8))   # 10분 = 80
    elif target_minutes <= 15:
        return max(80, int(target_minutes * 8))   # 15분 = 120
    else:
        return max(120, int(target_minutes * 7))  # 20분 = 140~150
...
def _parse_sections(full_text: str) -> list:
    ...
    for i in range(1, len(parts), 2):
        if i + 1 < len(parts):
            ...
            # 키워드 연동 영어 비주얼 프롬프트 자동 생성 (Kling image-to-video 최적화)
            prompt = _generate_visual_prompt(narration, pose)
    ...
    if not sections:
        default_pose = "neutral, calm and professional analyst pose"
        sections.append({
            "title": "인트로",
            "content": full_text.strip(),
            "prompt": _generate_visual_prompt(full_text.strip(), default_pose),
