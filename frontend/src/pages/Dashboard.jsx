      ...
      <div className="w-full max-w-none space-y-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => navigate('/shorts/new')} className="border border-slate-300 text-white rounded-xl px-4 py-2.5 text-sm font-semibold hover:border-accent-cyan">쇼츠 만들기</button>
        ...
        <section className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="p-5 border-b border-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-3 mt-4">
              <label className="relative xl:col-span-1"><Search size={16} className="absolute left-3 top-3 text-gray-500"/><input value={search} onChange={e => update(setSearch)(e.target.value)} placeholder="제목 또는 키워드" className="w-full rounded-xl bg-slate-100 border border-slate-300 pl-9 pr-3 py-2.5 text-sm text-slate-900" /></label>
...
function SummaryCard({ icon, label, value, onClick }) {
  return <button onClick={onClick} className="text-left bg-white border border-slate-200 hover:border-accent-cyan/60 rounded-xl p-5 transition"><div className="text-accent-cyan">{icon}</div><div className="text-2xl font-bold text-white mt-4">{value}</div><div className="text-sm text-gray-400 mt-1">{label}</div></button>
}
