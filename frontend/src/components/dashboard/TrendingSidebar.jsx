        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">대형 채널의 최근 영상 형식과 반응을 비교하세요.</p>
            </div>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto pt-4 pb-1">
          {TABS.map(tab => (
            <button
              ...
              className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-semibold transition ${activeTabId === tab.id ? 'border-red-500 bg-red-500 text-white' : 'border-navy-600 bg-navy-900 text-slate-500 hover:border-red-500/60 hover:text-slate-900'}`}
            >
              {tab.id === 'outperformer' && <TrendingUp size={12} className="inline mr-1 -mt-0.5" />}{tab.label}
            </button>
          ))}
        </div>
        <p className="mt-3 text-xs text-slate-500">{activeTab.description} · 라이브와 7일 이전 업로드는 제외</p>
      </div>

      {isLoading && <div className="flex flex-col items-center justify-center h-44 text-slate-500 text-sm"><div className="animate-spin rounded-full h-7 w-7 border-b-2 border-red-500 mb-3" />수집 중...</div>}
      {isError && <div className="text-center py-12 text-red-400 text-sm">트렌드 데이터를 불러오지 못했습니다.</div>}
      {!isLoading && !isError && videos.length === 0 && <div className="text-center py-12 text-slate-500 text-sm">이 조건에 맞는 최근 영상이 없습니다.</div>}
      ...
      {!isLoading && !isError && videos.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-3 p-4">
          {videos.map((video, idx) => {
            ...
            return (
              <button key={videoId || idx} onClick={() => videoId && window.open(`https://www.youtube.com/watch?v=${videoId}`, '_blank')} className="group relative text-left bg-navy-900/50 rounded-lg border border-slate-200 hover:border-red-500/60 transition overflow-hidden">
                <div className="aspect-video bg-navy-900 overflow-hidden relative">
                  {videoId ? <img src={`https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`} alt="" className="w-full h-full object-cover group-hover:scale-105 transition duration-300" /> : <div className="flex h-full items-center justify-center text-xs text-slate-500">썸네일 정보 없음</div>}
                ...
                <div className="p-3">
                  ...
                  <p className="text-[11px] text-slate-500 truncate mt-2">{channelTitle || '채널 정보 없음'}</p>
