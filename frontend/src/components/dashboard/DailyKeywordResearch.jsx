...
function ResearchSearchPanel({ keyword, onKeywordChange, activeTabId, onTabChange, onSearch, searching, videos, visibleVideos, page, onPageChange, scrollRef, onUseKeyword }) {
  ...
  return <div className="border-b border-navy-700 bg-navy-900/20 p-5"><div className="flex items-center gap-2"><Youtube size={17} className="text-red-400" /><h3 className="text-sm font-bold text-white">키워드·대형 채널 영상 탐색</h3></div><p className="mt-1 text-xs text-gray-400">조회수 반응이 높은 영상과 대형 채널의 최근 업로드 형식을 분리해 확인할 수 있습니다. 대형 채널 탭은 조회율이 낮아도 제외하지 않습니다.</p><div className="mt-3 flex gap-2 overflow-x-auto pb-1">{RESEARCH_TABS.map(tab => <button key={tab.id} type="button" onClick={() => onTabChange(tab.id)} className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-semibold transition ${activeTabId === tab.id ? 'border-red-500 bg-red-500 text-white' : 'border-navy-600 bg-navy-900 text-gray-400 hover:border-red-500/60 hover:text-white'}`}>{tab.id === 'outperformer' && <TrendingUp size={12} className="mr-1 inline -mt-0.5" />}{tab.label}</button>)}</div><p className="mt-2 text-[11px] text-gray-400">{activeTab.description} · 라이브와 7일 이전 업로드는 제외</p><div className="mt-3 flex flex-wrap gap-2"><input value={keyword} onChange={event => onKeywordChange(event.target.value)} onKeyDown={event => event.key === 'Enter' && keyword.trim() && onSearch()} placeholder="예: 삼성전자 실적, 코스피 전망, FOMC 금리" className="min-w-[260px] flex-1 rounded-lg border border-navy-600 bg-navy-900 px-3 py-2 text-xs text-white" /><button onClick={onSearch} disabled={searching || !keyword.trim()} className="inline-flex items-center gap-1.5 rounded-lg bg-accent-cyan px-3 py-2 text-xs font-bold text-navy-950 disabled:opacity-50">{searching ? <Loader size={14} className="animate-spin" /> : <Search size={14} />}영상 검색</button><button onClick={() => onUseKeyword?.(keyword.trim())} className="rounded-lg border border-accent-cyan/60 px-3 py-2 text-xs font-semibold text-accent-cyan">이 검색어로 작업 설정</button></div>{videos.length > 0 && <div ref={scrollRef} className="mt-4 space-y-3">{visibleVideos.map(video => <EvidenceVideoCard key={valueOf(video, 'videoId', 'video_id')} video={video} />)}<Pagination total={videos.length} currentPage={page} onChange={onPageChange} pageSize={PAGE_SIZE} /></div>}</div>
}
