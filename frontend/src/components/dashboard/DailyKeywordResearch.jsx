...
import { ArrowUpRight, Check, ChevronDown, ChevronUp, ExternalLink, Flame, Loader, Network, Plus, RefreshCw, Search, ThumbsUp, MessageCircle, TrendingUp, Youtube } from 'lucide-react'
import { useNavigate } from 'react-router-dom'
import apiClient from '../../api/client'
import { jobsApi } from '../../api/jobs'
import Pagination from '../Pagination'
import KeywordMindMap from './KeywordMindMap'
import StockMindMap from './StockMindMap'
...
const RESEARCH_TABS = [
  ...
  { id: '300k', label: '30만+ 채널', ranking: 'large_channel', minSubscribers: 300_000, description: '구독자 30만명 이상 채널의 최근 업로드' },
  { id: '500k', label: '50만+ 채널', ranking: 'large_channel', minSubscribers: 500_000, description: '구독자 50만명 이상 채널의 최근 업로드' },
  { id: '1m', label: '100만+ 채널', ranking: 'large_channel', minSubscribers: 1_000_000, description: '구독자 100만명 이상 채널의 최근 업로드' },
]
...
const displaySubscriberResponse = multiple => multiple == null ? '계산 불가' : `${(multiple * 100).toLocaleString('ko-KR', { maximumFractionDigits: 1 })}%`
...
function EvidenceVideoCard({ video, compact = false }) {
  ...
  return <article className="relative overflow-hidden rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
    ...
    <div className={`grid gap-3 ${compact ? 'lg:grid-cols-[minmax(0,1fr)_300px]' : 'xl:grid-cols-[minmax(0,1fr)_310px]'}`}>
      ...
      <div className="grid min-w-[290px] grid-cols-2 gap-2"><Metric label="구독자 대비 조회율">{metrics.multiple == null ? '계산 불가' : <span className={metrics.multiple >= 5 ? 'text-rose-700' : ''}>{metrics.multiple >= 5 && <Flame className="mr-1 inline" size={13} />}{displaySubscriberResponse(metrics.multiple)}</span>}</Metric><Metric label="게시 후 시간당 조회">{metrics.velocity == null ? '—' : `${displayNumber(metrics.velocity)}/시간`}</Metric><Metric label="좋아요율"><ThumbsUp className="mr-1 inline" size={13} />{metrics.likeRate == null ? '—' : `${(metrics.likeRate * 100).toFixed(2)}%`}</Metric><Metric label="댓글율"><MessageCircle className="mr-1 inline" size={13} />{metrics.commentRate == null ? '—' : `${(metrics.commentRate * 100).toFixed(2)}%`}</Metric></div>
    </div>
  </article>
}
...
function KeywordReason({ keyword, metric }) {
  ...
  return <article className="rounded-xl border border-violet-200 bg-violet-50/50 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><h5 className="font-semibold text-slate-900">{keyword}</h5><span className="rounded-full bg-violet-100 px-2 py-1 text-[11px] font-bold text-violet-700">최고 조회율 {displaySubscriberResponse(metric?.bestMultiple)}</span></div><p className="mt-1 text-[11px] text-slate-700">반복 태그: {(metric?.matchedTags || []).length ? (metric.matchedTags || []).map(tag => `#${tag}`).join(' · ') : '공개 제목·태그 공통어'}</p><div className="mt-2 space-y-2">{evidence.slice(0, 3).map(video => <div key={video.videoId} className="rounded-lg border border-slate-200 bg-white p-2"><p className="line-clamp-1 text-xs font-semibold text-slate-800">{video.title}</p><p className="mt-1 text-[10px] text-slate-600">{video.channelTitle} · 구독자 {displayNumber(video.subscribers)} · 조회 {displayNumber(video.views)} · 구독자 대비 조회율 {displaySubscriberResponse(video.bestMultiple)}</p><VideoTags tags={video.matchedTags} className="mt-1" /></div>)}</div></article>
}
...
function ResearchSearchPanel({ keyword, onKeywordChange, activeTabId, onTabChange, onSearch, searching, videos, visibleVideos, page, onPageChange, scrollRef, onUseKeyword }) {
  const activeTab = RESEARCH_TABS.find(tab => tab.id === activeTabId) || RESEARCH_TABS[0]
  return <div className="border-b border-slate-200 bg-slate-50/40 p-5 lg:p-6"><div className="flex items-center gap-2.5"><div className="p-1.5 rounded-lg bg-red-50 text-red-600"><Youtube size={18} /></div><div><h3 className="text-base font-bold text-slate-900">키워드·대형 채널 영상 탐색</h3><p className="mt-0.5 text-xs text-slate-500">조회수 반응이 높은 영상과 대형 채널의 최근 업로드 형식을 분리해 확인할 수 있습니다.</p></div></div><div className="mt-4 flex gap-2 overflow-x-auto pb-1.5">{RESEARCH_TABS.map(tab => <button key={tab.id} type="button" onClick={() => onTabChange(tab.id)} className={`shrink-0 rounded-lg border px-3.5 py-1.5 text-xs font-semibold transition ${activeTabId === tab.id ? 'border-red-600 bg-red-600 text-white shadow-sm' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:bg-slate-50'}`}>{tab.id === 'outperformer' && <TrendingUp size={13} className="mr-1 inline -mt-0.5" />}{tab.label}</button>)}</div><p className="mt-2 text-xs text-slate-500">{activeTab.description} · 라이브와 7일 이전 업로드는 제외</p><div className="mt-3.5 flex flex-wrap gap-2"><input value={keyword} onChange={event => onKeywordChange(event.target.value)} onKeyDown={event => event.key === 'Enter' && keyword.trim() && onSearch()} placeholder="예: 삼성전자 실적, 코스피 전망, FOMC 금리" className="min-w-[260px] flex-1 rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-xs text-slate-900 placeholder:text-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 shadow-sm" /><button onClick={onSearch} disabled={searching || !keyword.trim()} className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 px-4 py-2.5 text-xs font-bold text-white shadow-sm disabled:opacity-50 transition">{searching ? <Loader size={15} className="animate-spin" /> : <Search size={15} />}영상 검색</button><button onClick={() => onUseKeyword?.(keyword.trim())} className="rounded-xl border border-indigo-200 bg-indigo-50/60 hover:bg-indigo-100/60 px-3.5 py-2.5 text-xs font-semibold text-indigo-700 transition">이 검색어로 작업 설정</button></div>{videos.length > 0 && <div ref={scrollRef} className="mt-5 space-y-3">{visibleVideos.map(video => <EvidenceVideoCard key={valueOf(video, 'videoId', 'video_id')} video={video} />)}<Pagination total={videos.length} currentPage={page} onChange={onPageChange} pageSize={PAGE_SIZE} /></div>}</div>
}
...
export default function DailyKeywordResearch({ onUseKeyword }) {
  ...
  const [activeResearchTabId, setActiveResearchTabId] = useState(RESEARCH_TABS[0].id)
  ...
  const searchVideos = useMutation({ mutationFn: request => jobsApi.trendingYoutube(request.keyword, request), onSuccess: result => { setResearchVideos(Array.isArray(result) ? result : result?.videos || []); setDirectPage(1); setMindmap(null); setSelected({}) } })
  ...
  const directTrustedVideos = researchVideos
  ...
  const searchCurrentTab = () => {
    ...
    if (researchKeyword.trim()) searchVideos.mutate({ keyword: researchKeyword.trim(), ...activeTab })
  }
  const selectResearchTab = tabId => {
    setActiveResearchTabId(tabId)
    const tab = RESEARCH_TABS.find(item => item.id === tabId) || RESEARCH_TABS[0]
    if (researchKeyword.trim()) searchVideos.mutate({ keyword: researchKeyword.trim(), ...tab })
  }
  ...
  return <section className={`overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm ${selectedKeywords.size > 0 ? 'pb-28' : ''}`}>
    ...
    <div className="border-b border-slate-200 p-5 lg:p-6 mb-2">
      <StockMindMap onSelectKeyword={(kw) => toggleKeyword(kw)} selectedKeywords={[...selectedKeywords]} />
    </div>
    <div className="border-b border-slate-200 bg-white p-5 lg:p-6"><div className="flex items-center gap-2.5"><div className="p-1.5 rounded-lg bg-indigo-50 text-indigo-600"><Network size={18} /></div><div><h3 className="text-base font-bold text-slate-900">{researchVideos.length ? `“${researchKeyword}” 실시간 주제 지도` : '오늘의 주식 기회 지도'}</h3><p className="mt-0.5 text-xs text-slate-500">최근 7일의 일반 영상 기준 지표 분석 데이터입니다.</p></div></div>{makeMindmap.isPending && <p className="mt-4 rounded-xl bg-slate-50 p-5 text-center text-xs font-medium text-slate-500 border border-slate-200 animate-pulse">공통 태그와 근거 영상을 분석하는 중입니다...</p>}{mindmap && <><KeywordMindMap mindmap={mindmap} selectedKeywords={selectedKeywords} onToggle={toggleKeyword} evidenceVideos={premiumVideos} /><div className="mt-4 rounded-xl border border-indigo-100 bg-indigo-50/40 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><h4 className="text-sm font-bold text-slate-900">스크립트용 정제 키워드 후보</h4><p className="mt-0.5 text-xs text-slate-600">근거 영상이 명확한 키워드만 최대 5개까지 선택합니다.</p></div><span className="rounded-full bg-indigo-100 px-2.5 py-1 text-xs font-bold text-indigo-700">{selectedKeywords.size}/{MAX_SELECTED_KEYWORDS}</span></div><div className="mt-3 flex flex-wrap gap-2">{(mindmap.primary || []).slice(0, 8).map(node => <button key={node.keyword} type="button" onClick={() => toggleKeyword(node.keyword, node)} className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition ${selectedKeywords.has(node.keyword) ? 'border-indigo-600 bg-indigo-600 text-white shadow-sm' : 'border-slate-300 bg-white text-slate-800 hover:border-indigo-400 hover:bg-indigo-50/50'}`}>{selectedKeywords.has(node.keyword) && <Check size={13} className="mr-1 inline" />}{node.keyword} · 최고 조회율 {displaySubscriberResponse(node.bestMultiple)}</button>)}</div>{selectedKeywords.size >= MAX_SELECTED_KEYWORDS && <p className="mt-2 text-xs font-medium text-amber-700">최대 5개가 선택되었습니다. 다른 후보를 선택하려면 하나를 해제하세요.</p>}<div className="mt-3 grid gap-2 lg:grid-cols-2">{Object.entries(selected).map(([keyword, item]) => <KeywordReason key={keyword} keyword={keyword} metric={item.metric} />)}</div></div></>}</div>
    <div ref={dailyRef} className="border-b border-slate-200 p-5 lg:p-6"><h3 className="text-base font-bold text-slate-900">마인드맵 근거 영상</h3><p className="mt-0.5 text-xs text-slate-500">최근 7일 일반 영상 중 수집 기준을 충족한 공개 YouTube 지표입니다.</p>{isLoading && <p className="py-10 text-center text-xs text-slate-500 font-medium">오늘의 고성과 영상을 불러오는 중입니다...</p>}{isError && <p className="py-10 text-center text-xs text-rose-600 font-semibold">자동 후보를 불러오지 못했습니다.</p>}{!isLoading && !isError && dailyVisible.length === 0 && <div className="mt-4 py-10 px-4 text-center rounded-xl border border-dashed border-slate-200 bg-slate-50/50"><p className="text-sm font-semibold text-slate-700">오늘 수집된 기준 충족 영상이 없습니다.</p><p className="mt-1 text-xs text-slate-500">아래 키워드·대형 채널 탐색에서 다른 검색어로 넓게 확인해 보세요.</p></div>}<div className="mt-4 space-y-3">{dailyVisible.map((video, index) => { const item = dailyItems.find(candidate => candidate.keyword === video.title) || {}; const key = `${video.title}-${index}`; const sources = valueOf(item, 'sourceVideos', 'source_videos') || []; return <div key={key}><EvidenceVideoCard video={video} compact /><div className="mt-1.5 flex justify-end gap-3 text-xs font-medium"><button onClick={() => setExpanded(expanded === key ? null : key)} className="inline-flex items-center gap-1 text-slate-500 hover:text-slate-800">근거 세부 보기 ({sources.length}) {expanded === key ? <ChevronUp size={13} /> : <ChevronDown size={13} />}</button><button onClick={() => onUseKeyword?.(video.title)} className="inline-flex items-center gap-1 text-indigo-600 hover:underline font-semibold">이 주제로 작업 설정 <ArrowUpRight size={13} /></button></div>{expanded === key && <div className="mt-2 space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-3">{sources.map(source => <EvidenceVideoCard key={valueOf(source, 'videoId', 'video_id')} video={source} compact />)}</div>}</div>})}</div><Pagination total={dailyTrustedVideos.length} currentPage={dailyPage} onChange={page => movePage(setDailyPage, page, dailyRef)} pageSize={PAGE_SIZE} /></div>
