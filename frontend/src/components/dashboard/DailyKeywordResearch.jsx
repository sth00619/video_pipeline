...
import { ArrowUpRight, Plus, RefreshCw, Youtube } from 'lucide-react'
...
export default function DailyKeywordResearch({ onUseKeyword }) {
  ...
  return (
    <section className="bg-navy-800 rounded-xl border border-navy-700 overflow-hidden">
      ...
      <Pagination total={items.length} currentPage={page} onChange={setPage} pageSize={PAGE_SIZE}/><div className="px-5 py-3 text-[10px] text-gray-500 border-t border-navy-700 flex items-center gap-1"><Youtube size={12}/>공개 YouTube Data API 지표만 표시합니다. 타 채널의 평균 시청 시간·CTR은 공개 API에서 제공되지 않아 unavailable입니다.</div>
    </section>
  )
}
