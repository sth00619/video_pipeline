...
### 목표 3 — 이미지 의미·화풍·텍스트·수치·캐릭터 정확성: 약 68% (scene07 육안 회귀 반영)
...
- 정정: scene07 HTTP 200 canary에서 해부학 붕괴·화풍 이탈·장면 의미 전달 실패가 사용자 육안 검토로 확인됐다. 기존 정량 얼굴/OCR 게이트만으로는 전체 시각 성공을 보장하지 못하므로 82% 추정은 철회한다.
- 남음: 유료 canary 실물 첨부·사용자 육안 게이트, scene15 의사문자, 실제 v2 얼굴 성공 표본, 다양한 배경의 물리 표면 성공률, 다주제 이미지 분포 검증
